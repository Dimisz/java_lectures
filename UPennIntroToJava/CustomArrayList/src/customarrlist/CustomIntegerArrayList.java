...
public class CustomIntegerArrayList {
	...
	public int remove(int index) {
		// TODO Implement method
		int removedElement = arr.get(index);
		arr.remove(index);
	...
	public ArrayList<Integer> splice(int index, int num) {
		...
		if(index < 0 || index >= arr.size() || num == 0){
			...
			for(int i = index; i < index + num; i++){
				if(index < arr.size()){
					int elementToRemove = arr.get(index);
					removedElements.add(elementToRemove);
					System.out.println("Removing element: " + elementToRemove + " at: " + i);
					arr.remove(index);
				}
			}
		}
	...
	public ArrayList<Integer> splice(int index, int num, int[] otherArray) {
		...
		if(index < 0 || index >= arr.size() || num == 0){
			...
			for(int i = index; i < index + num; i++){
				if(i < arr.size()){
					int elementToRemove = arr.get(i);
					removedElements.add(elementToRemove);
					arr.remove(i);
				}
			}
			for(int j = 0; j < otherArray.length; j++){
				arr.add(index + j, otherArray[j]);
			}
		}
		return removedElements;
	}
	...
	public static void main(String args[]) {
		...
		arr1.add(2);
		arr1.add(7);
		System.out.println(arr1.getArrayList());// [3, 6, 2, 7]
